...
As well as the sequence data files (which end in ```.fastq.gz```) there are also files that end in ```.md5```. These were generated by ```md5sum``` by the sequencing centre. This generates a unique string of numbers and letters corrspondonding to the characteristics of the file. If we regenerate this code then it should be exactly the same as it the code egenrated by the sequencing centre. If it is different then this indicates that the file has become corrupted, which can happen when it is being moved or dowloaded. 
