...
You might notice that the contents of the directory you are currently in are also displayed on the right hand panel in MobaXterm and you can use the icons to navigate the directories in sharc. 
...
We are going to create a job to submit to the cluster that will calculate the md5 hash for all 6 files data files for this individual. 

Copy the following file into your fastdata directory
```/usr/local/extras/Genomics/workshops/NGS_AdvSta_2019/scripts/MD5calc.sh```
open it with ```nano```
```
#!/bin/bash
#$ -l h_rt=2:00:00
...
### Excercise
Try to use the command ```cat``` to view all the ```.md5``` files at once (hint, you will need to use ```*```)
